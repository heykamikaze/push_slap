...
/*   Updated: 2022/05/17 20:02:21 by nbenjami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_get_line(char *remains)
{
	...
	while (remains[i])
		if (remains[i++] == '\n')
			break ;
	before_n = ft_substr_gnl(remains, 0, i);
	return (before_n);
}
...
char	*ft_get_line_after_n(char *remains)
{
	...
	if (remains[i] == '\0')
	{
		...
		after_n = ft_substr_gnl(remains, i, ft_strlen_gnl(remains + i));
	free (remains);
	return (after_n);
}
...
char	*ft_start(int fd, char *remains)
{
	...
	if (!buffer)
		return (NULL);
	reader = 1;
	while (reader > 0 && !ft_strchr_gnl(remains, '\n'))
	{
